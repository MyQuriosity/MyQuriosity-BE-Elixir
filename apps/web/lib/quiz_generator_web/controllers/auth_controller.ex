defmodule Web.AuthController do
  ...
  alias Web.HeaderUtils
  alias Web.AuthContext
  alias Web.UserContext
  alias Web.Utils.Auth
  alias Web.SharedView
  alias Web.OtpCodeContext
  alias Web.OtpUtils
  ...
  def reset_password(
    ...
    query =
      from(oc in Web.OtpCode,
        inner_join: user in Web.User,
    ...
    case otp do
      ...
      _ ->
        multi_result =
          ...
          |> Ecto.Multi.update(
            :user,
            Web.User.update_password_changeset(otp.user, %{
              password: password
            })
          )
          |> Ecto.Multi.update(
            :otp,
            Web.OtpCode.changeset(otp, %{
              deactivated_at: DateTime.utc_now()
            })
          )
          |> Web.Repo.transaction()
  ...
  defp authenticate_user(conn, user, true) do
    ...
    {syllabus_providers, _meta} =
      Web.SyllabusProviderContext.fetch_active_paginated(%{})
